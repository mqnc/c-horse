...
* Semantic values
*/
struct SemanticValues : protected std::vector<any> // return values of rules
{
    // Input text
    const char* path;
    const char* ss;
...
any call(F fn, Args&&... args) // calls functions
 

class Action // wraps functions:  std::function<any (const SemanticValues& sv, any& dt)> Fty;
{
public:
    Action() = default;
...
public:
    ...
    size_t                         id;
    Action                         action;
    std::function<void (std::string s, any& dt)>  enter; // patch
    std::function<void (std::string s, bool match, any& dt)>  leave;
...
inline size_t Holder::parse(const char* s, size_t n, SemanticValues& sv, Context& c, any& dt) const {
    ...
    c.packrat(s, outer_->id, len, val, [&](any& a_val) {
        ...
        if (outer_->enter) {
            outer_->enter(std::string(s, n), dt); // patch
        }

        const auto& rule = *ope_;
        len = rule.parse(s, n, chldsv, c, dt);
		auto match = success(len);

		auto se2 = make_scope_exit([&]() {
			c.pop();

			if (outer_->leave) {
				outer_->leave(std::string(s, n), match, dt);
			}
		});
